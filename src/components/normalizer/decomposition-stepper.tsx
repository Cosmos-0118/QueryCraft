...
export function DecompositionStepper({
  ...
  return (
    <div className={cn('rounded-2xl border border-border/80/50 bg-muted/60', className)}>
      <div className="flex items-center gap-2 border-b border-border/80/40 bg-muted/80/30 px-4 py-3 sm:px-5">
      ...
      {/* Step tabs */}
      <div className="flex gap-1.5 overflow-x-auto border-b border-border/80/40 px-4 py-3 sm:px-5">
        {steps.map((step, i) => (
          <button
            key={i}
            ...
            className={cn(
              ...
              i === activeStep
                ...
                : 'text-muted-foreground/80 hover:bg-muted/80/60 hover:text-foreground/80',
            )}
          >
            <span
              className={cn(
                ...
                i === activeStep
                  ...
                  : 'bg-muted/80/50 text-muted-foreground',
              )}
            >
              {i + 1}
            </span>
      ...
      {active && (
        <div className="space-y-5 p-4 sm:p-5">
          {/* Explanation */}
          <div className="rounded-xl border border-border/80/40 bg-muted/80/20 px-4 py-3">
            <p className="text-sm leading-relaxed text-foreground/80">{active.explanation}</p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-muted-foreground">{active.tables.length} table{active.tables.length !== 1 ? 's' : ''}</span>
            </div>
            ...
            {active.tables.map((table) => (
              <div
                key={table.name}
                className="rounded-xl border border-border/50 bg-muted/80/20 p-3.5"
              >
                <div className="mb-3 flex items-center gap-2">
                  <span className="text-sm font-bold text-foreground/90">{table.name}</span>
                      ...
                      <span
                        ...
                        className={cn(
                          ...
                          isPK
                            ...
                            : 'border border-border/80/40 bg-muted/80/50 text-muted-foreground',
                        )}
                      >
                ...
                {table.sampleData && table.sampleData.length > 0 && (
                  <div className="mt-3 overflow-x-auto rounded-lg border border-border/60">
                    <table className="w-full min-w-[260px] text-[10px]">
                      <thead>
                        <tr className="border-b border-border/60 bg-muted/80/40">
                          {table.columns.map((col) => (
                            <th
                              key={col}
                              className="px-2 py-1.5 text-left font-mono font-semibold text-muted-foreground/80"
                            >
                              {col}
                            </th>
                          ))}
                    ...
                    <ul className="space-y-1.5">
                      {table.functionalDependencies.map((fd, fi) => (
                        <li key={fi} className="flex items-center gap-1 font-mono leading-relaxed">
                          ...
                          <ArrowRight className="h-2.5 w-2.5 shrink-0 text-muted-foreground/80" />
                          <span className="text-muted-foreground">{fd.dependent.join(', ')}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
