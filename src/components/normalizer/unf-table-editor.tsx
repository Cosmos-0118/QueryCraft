    ...
    <div className={cn('rounded-xl border border-border/80/50 bg-muted/60', className)}>
      ...
      <div className="space-y-4 p-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-border/80/60 bg-muted/80/40 px-3 py-2 font-mono text-sm text-foreground/90 outline-none transition-colors placeholder:text-muted-foreground focus:border-cyan-500/40 focus:ring-1 focus:ring-cyan-500/20"
            />
          </div>
        ...
        <div className="space-y-2.5">
          ...
          {columns.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border/80/50 bg-muted/80/20 px-4 py-5 text-center text-xs text-muted-foreground/80">
            ...
            <div className="overflow-x-auto rounded-lg border border-border/50">
              <table className="w-full min-w-[480px] text-xs">
                <thead>
                  <tr className="border-b border-border/50 bg-muted/80/30">
                    <th className="w-12 px-2 py-2 text-left text-[10px] uppercase tracking-wider text-muted-foreground">#</th>
                    {columns.map((column) => (
                      <th
                        key={column}
                        className="px-2 py-2 text-left font-mono text-[11px] font-semibold text-muted-foreground"
                      >
                        {column}
                      </th>
                    ))}
                  ...
                  {normalizedRows.length === 0 ? (
                    ...
                    normalizedRows.map((row, rowIndex) => (
                      <tr
                        ...
                        className="border-b border-border/35 last:border-b-0 hover:bg-muted/80/20"
                      >
                        <td className="px-2 py-1.5 text-center text-[10px] text-muted-foreground">{rowIndex + 1}</td>
                        {row.map((cell, columnIndex) => (
                          <td key={`${rowIndex}-${columnIndex}`} className="px-1 py-1.5">
                            <input
                              ...
                              placeholder="value"
                              className="w-full rounded-md border border-border/80/60 bg-muted/80/50 px-2 py-1 font-mono text-[11px] text-foreground/90 outline-none transition-colors placeholder:text-muted-foreground focus:border-cyan-500/40"
                            />
                          </td>
                        ))}
                        <td className="px-2 py-1.5 text-right">
                          <button
                            ...
                            className="inline-flex h-6 w-6 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-red-500/10 hover:text-red-400"
