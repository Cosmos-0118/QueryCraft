    ...
    return (
      <div className={cn('rounded-xl border border-border/80/50 bg-muted/60', className)}>
        ...
        <div className="px-4 py-8 text-center">
          <div className="mx-auto mb-2 flex h-8 w-8 items-center justify-center rounded-lg bg-muted/80/60">
            <Clock className="h-4 w-4 text-muted-foreground" />
          </div>
          <p className="text-xs text-muted-foreground">No queries executed yet.</p>
          <p className="mt-0.5 text-[10px] text-muted-foreground/80">Run a query to see it here</p>
        </div>
      </div>
    );
  }
            ...
            >
              ...
              <div className="flex items-center gap-1.5 px-3 py-2">
                <button
                  ...
                  className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted/80/60 hover:text-muted-foreground"
                >
                  {isExpanded ? (
                ...
                >
                  <code className="line-clamp-1 block font-mono text-[11px] text-foreground/80">
                    {entry.query}
                  </code>
                </button>
                <button
                  onClick={() => handleCopy(entry.query, i)}
                  className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md text-muted-foreground/80 transition-colors hover:bg-muted/80/60 hover:text-muted-foreground"
              ...
              <div className="flex items-center gap-2 px-3 pb-1.5 pl-[2.75rem]">
                ...
                <span className="rounded bg-muted/80/70 px-1.5 py-0.5 text-[10px] text-muted-foreground/80">
                  {entry.database || 'main'}
                </span>
                {entry.result && (
                  <>
              ...
              {isExpanded && (
                <div className="mx-3 mb-3 mt-1 space-y-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => onSelect(entry.query, entry.database || 'main')}
                      className="inline-flex items-center gap-1 rounded-md bg-muted/80/50 px-2.5 py-1 text-[10px] font-medium text-muted-foreground transition-colors hover:bg-muted/80 hover:text-foreground/90"
                    >
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border/60">
      {statementResults.length > 1 && (
        <div className="flex items-center gap-1 overflow-x-auto border-b border-border/50 bg-muted/60 px-2 py-1.5">
            ...
            <button
              ...
              className={cn(
                ...
                idx === activeIndex
                  ...
                  : 'border-border/80/60 bg-muted/80/50 text-muted-foreground/80 hover:text-foreground/80',
              )}
            >
              S{idx + 1}
            </button>
      ...
      <div className="border-b border-border/50 bg-muted/80/20 px-3 py-1.5">
        <div className="flex items-center gap-1.5">
      ...
      {active.error ? (
          ...
          <table className="w-full min-w-[300px] text-[11px]">
            <thead>
              <tr className="border-b border-border/50 bg-muted/50">
                {active.columns.map((col) => (
                  <th
                    key={col}
                    className="whitespace-nowrap px-3 py-1.5 text-left font-mono text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/80"
                  >
                    {col}
                  </th>
                ))}
            ...
            <tbody>
              {active.rows.map((row, ri) => (
                <tr
                  key={ri}
                  className="border-b border-border/30 last:border-0 transition-colors hover:bg-muted/80/20"
                >
                  {active.columns.map((col) => (
                    <td
                      key={col}
                    ...
                    >
                      {row[col] === null || row[col] === undefined ? (
                        <span className="text-muted-foreground/80 italic">NULL</span>
                      ) : (
                        String(row[col])
                      )}
                    </td>
        ...
        <div className="flex items-center gap-2 rounded-b-lg border-t border-border/60 bg-emerald-500/5 px-3 py-2.5">
