...
export function TableBrowser({ open, onClose, tables, execute }: TableBrowserProps) {
  ...
  const previewData = selectedTable
    ? execute(`SELECT * FROM "${selectedTable}" LIMIT 100`)
    : null;

  const getRowCount = (name: string) => {
    const r = execute(`SELECT COUNT(*) as c FROM "${name}"`);
    return r.rows.length > 0 ? Number(r.rows[0].c) : 0;
  };
